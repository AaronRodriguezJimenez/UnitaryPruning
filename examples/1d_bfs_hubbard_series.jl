...
function run(; Lx=2, U=0, w_type = "Majorana", max_weight=1)
   
    N = 2*Lx
    ket = Ket(N,0)
    o = Pauli(N, Z=[1])
    
    loc = "/home/aaronrodriguez/DATA_models/FH_D-1.hdf5"
...
function plot_series_and_save_results(; 
    LXs = [2, 3, 4, 5, 6], 
    Us = [0, 2, 4, 8, 12], 
    output_dir = "HUBBARD_series_results",
            ...
            for U in Us
                ...
                for w_type in ["Pauli", "Majorana"]
                    for mw in max_weights
                        ...
                        err = run(Lx = L, U = U, w_type = w_type, max_weight = mw)
