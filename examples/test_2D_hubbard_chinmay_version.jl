...
function run(; Lx = 2, Ly = 2, t = 1.0, U = 2.0, k=1 , w_type = "Majorana", max_weight=1)
    ...
    U = UnitaryPruning.build_time_evolution_matrix_fast(N, generators, parameters)
    o_mat = Matrix(o)
    m = diag(U'*o_mat*U)
    #expval = m[1]

    abs_err = abs(real(m[1]) - real(ei))
    println("Exact :", real(m[1]), " Approx :", real(ei), " Abs Err: ", abs_err)
    return abs_err
end
...
function plot_abs_error_vs_weight_pdf(; Lx = 2, Ly = 2, t = 1.0, U = 2.0, k=1, max_weights=0:2:6)
    ...
    title!(" L = $Lx, U=$U, t=-$t, k=$k")
    #title!("Error vs Max Weight Cutoff (Hubbard, U=$U, t=-$t)")

    #filename="2D_Hubbard_test_energy_Lx=$Lx-Ly=$Ly-k=$k-CH.pdf"
    filename="TEST-2D_Hubbard_test_abs_error_vs_weight_Lx=$Lx-U=$U-k=$k-CH.pdf"
    savefig(plt, filename)
    println("Plot saved as $filename")
end
    
plot_abs_error_vs_weight_pdf(Lx=2, Ly=2, t=1.0, U=2.0, k=10, max_weights=1:1:8)
