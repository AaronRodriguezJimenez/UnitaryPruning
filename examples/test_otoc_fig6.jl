...
"""
 ...
 Resutls from the paper use a time step dt=0.05 and simulate up to 30 Trotter layers, the results
 are intended to show the different <Z_{21} Z_{22}> values obtained with PP using different choices
 of the minimum absolute coefficient threshold:
   - 2^-12
   - 2^-13
   - 2^-14
   - 2^-15
   ...
   and imposing an additional Pauli weight truncation of 9. Authors observe "apparent convergence in the 
   sense that the different simulations agree until longer and longer times which increases trust in certain
   features in time."
...
function run1D(; N=6, threshold=1e-3, dt=0.1, T=10.0, scheme=:mag,
    ...
    o   = Pauli(N, Z=[1])
    gens, params = tilted_field_ising_hamiltonian(N, J, B, theta, k=k)
...
thresholds = [1e-1, 1e-2, 1e-3, 1e-4, 2e-12] 
curves = [] 
NQubits = 4 
total_time = 1.5 
wmax = 6
for th in thresholds
     push!(curves, run1D(N=NQubits, threshold=th, dt=0.05, T=total_time, scheme=:combined, wmax=wmax, saveprefix="mag_prune"))
end 
...
xlabel!("Time"); ylabel!("<Z>"); title!("Max weigth = $wmax")
savefig(plt, "N=$(NQubits)_fig6_coeff_thresh_pruning.pdf")
=# 

# 2D tests
# - - - Threshold+Weight Based Pruning Comparisons - - - #
thresholds = [1e-4, 2e-5, 2e-6, 2e-7] 
curves = [] 
NQubitsX = 2
NQubitsY = 2
total_time = 1.5 
wmax = 4
for th in thresholds
     push!(curves, run2D(Nx=NQubitsX, Ny=NQubitsY, threshold=th, dt=0.05, T=total_time, scheme=:combined, wmax=wmax, saveprefix="mag_prune"))
end 
...
savefig(plt, "Nx=$(NQubitsX)_Ny=$(NQubitsY)_fig6_coeff_thresh_pruning.pdf")
