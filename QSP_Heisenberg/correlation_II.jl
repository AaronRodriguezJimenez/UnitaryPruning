...
function trotter_step_1st_order_DBf!(W::PauliSum{N, T},
	...
	cutoff = 1e-8,
	error_accumulation = false) where {N, T}
...
"""
 Saves computed correlation data to a CSV file.
 columns: Time, C_z1z2, C_z1z3, ...
"""
...
function load_and_run_corr(Nx = 2, Ny=2, Jx = 1.0, Jy = 1.0, Jz=1.0,
							dt = 0.05, t_max = 2.0,
							cutoff = 1e-6,
    ...
    # --- 1. Load Hamiltonian parameters ---
	N = Int(Nx * Ny)
    # --- 2. Build Hamiltonian ---
    println("\nBuilding Hamiltonian...")
    #H_total_sum = ising_hyperbolic_pauli(adj_matrix, Jzz, Jx, m)
	H_total_sum = heisenberg_2D(Nx, Ny, Jx, Jy, Jz; x=0, y=0, z=0, periodic=false)
    H_terms = build_H_terms(H_total_sum; order_type = order_type)

    # --- 3. Define Operators ---
    println("\nInitializing W(0) = Z_1 and V sites...")
    i_site = 1
    V_sites = [1,2,3,4] # You can expand this array to include more sites like [2,3,4,5]
    ...
    for step in 0:steps
        ...
        elseif trotter_order == 1
			#println(typeof(W_sum))
			#println(typeof(H_terms))
			#println(typeof(dt))
			#println(typeof(V_ops_vec))
			#println(typeof(cutoff))
			#println(typeof(errorsor_check))
			#println("Running first-order DBF with tracking...")
        ...
        for (idx, j) in enumerate(V_sites)
			...
			str = @sprintf("%8.5f  %8.f  %8.2e", corr_val, corrected_val, err_val)
        ...
        # --- C. Extra Clipping ---
        if max_maj_weight > 0
            DBF.majorana_weight_clip!(W_sum, max_maj_weight)
        end
        if max_pauli_weight > 0
            DBF.weight_clip!(W_sum, max_pauli_weight)
        end
...
time_data, corr_data, V_sites = load_and_run_corr(nx, ny, jx, jy, jz,
							dt, tmax,
							cutoff,
							max_maj_weigh,
							max_pauli_weight,
...
save_corr_csv("test.csv", time_data, corr_data, V_sites)

# Plot C(t) results
function make_plot(time_data, corr_data, V_sites, cutoff)
    plt = plot(title="Correlation Function C(t) = <Z1(t) V(0)>", legend=:topright)

    for site in V_sites
        plot!(plt, time_data, corr_data[site], lw=2,
              label="(Z, th=$cutoff")
    end
...
make_plot(time_data, corr_data, V_sites, cutoff)